...
int main() {
    ...
    Mat diff(game->getScreenHeight() * 0.8, game->getScreenWidth(), CV_8UC3);
    Mat now(game->getScreenHeight() * 0.8, game->getScreenWidth(), CV_8UC3);
    Mat1b prev(game->getScreenHeight() * 0.8, game->getScreenWidth(), CV_8UC3);
    Mat3b lbl(game->getScreenHeight() * 0.8, game->getScreenWidth(), Vec3b(0, 0, 0));
    const int d = 140;
    ...
    for (int i = 0; i < 20; ++i) {
        ...
        while (!game->isEpisodeFinished()) {
            ...
            for (int k = 0; k < nLabels; ++k) {
                ...
                bool a = wdt > hgt - (wdt / (hgt != 0 ? hgt : 99999999.0));
            ...
            if (nLabels <= 0) {
                ...
                for (auto &j : goout) {
                    if (j.x >= game->getScreenWidth() / 2 - 45 and
                        j.x <= game->getScreenWidth() / 2 + 45) {
                        game->makeAction(didaction);
                    }
                }
                if (middles[K].x < game->getScreenWidth() / 2 - 20) { // 20 to max
                    game->makeAction({1, 0, 1});
                    didaction = {1, 0, 0};
                } else if (middles[K].x > game->getScreenWidth() / 2 + 20) {
                ...
                Mat3b lbbl(game->getScreenHeight(), game->getScreenWidth(), Vec3b(0, 0, 0));
                for (int k = 0; k < pts.size(); ++k) {
                    lbbl(pts[k]) = colors[labls[k]];
                }
